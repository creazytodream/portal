...
public class ExcelUtil {
	...
	public static <T> void listToExcel(List<T> list,
			...
			int sheetSize, OutputStream out) throws OldExcelFormatException {

		if (list == null || list.size() == 0) {
			throw new OldExcelFormatException("数据源中没有任何数据");
		}
		...
		try {
			...
			e.printStackTrace();
			// 如果是OldExcelFormatException，则直接抛出
			if (e instanceof OldExcelFormatException) {
				throw (OldExcelFormatException) e;

				// 否则将其它异常包装成OldExcelFormatException再抛出
			} else {
				throw new OldExcelFormatException("导出Excel失败");
			}
		}
	...
	public static <T> void listToExcel(List<T> list,
			...
			OutputStream out) throws OldExcelFormatException {
	...
	public static <T> void listToExcel(List<T> list,
			LinkedHashMap<String, String> fieldMap, String sheetName,
			int sheetSize, HttpServletResponse response) throws OldExcelFormatException {
	...
	public static <T> void listToExcel(List<T> list,
			...
			HttpServletResponse response) throws OldExcelFormatException {
...
//								/ new Integer(sheetSize).doubleValue());
	...
	public static <T> List<T> excelToList(InputStream in, String sheetName,
			...
			String[] uniqueFields) throws OldExcelFormatException {
		...
		try {
			...
			// 如果Excel中没有数据则提示错误
			if (realRows <= 1) {
				throw new OldExcelFormatException("Excel文件中没有任何数据");
			}
			...
			// 如果有列名不存在，则抛出异常，提示错误
			if (!isExist) {
				throw new OldExcelFormatException("Excel中缺少必要的字段，或字段名称有误");
			}
			...
			for (int i = 1; i < realRows; i++) {
				...
				if (nullCols == uniqueFields.length) {
					throw new OldExcelFormatException("Excel中有重复行，请检查");
				}
			}
			...
			if (e instanceof OldExcelFormatException) {
				...
				e.printStackTrace();
				throw new OldExcelFormatException("导入Excel失败");
			}
		}
		return resultList;
	}
	...
	public static Object getFieldValueByName(String fieldName, Object o)
		...
		if (field != null) {
			...
			throw new OldExcelFormatException(o.getClass().getSimpleName() + "类不存在字段名 "
					+ fieldName);
		}
